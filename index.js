...
//Mappings of Human Readable -> Array
const mappings={
    universal:{
        types:["Normal","Favorite","Foreign","Special"],
        gender:["Male","Female"],
        favoriteColors:["Red","Orange","Yellow","Lime","Green","Blue","Cyan","Pink","Purple","Brown","White","Black"],
        skinColors:["White","Tanned White","Darker White","Tanned Darker","Mostly Black","Black"],
        hairColors:["Black","Brown","Red","Reddish Brown","Grey","Light Brown","Dark Blonde","Blonde"],
        eyeColors:["Black","Grey","Brown","Lime","Blue","Green"]
    },
    wii:{
...
function convertMii(jsonIn,typeTo){
    typeFrom=jsonIn.meta.type?.toLowerCase();
...
async function writeWiiBin(mii, outPath){
    if(mii.console?.toLowerCase() !== "wii"){
        mii=convertMii(mii);
    }
    var miiBin="0";
    ...
    let miiId=mii.meta.miiId;
    if(mii.meta.miiId==undefined||mii.meta.miiId?.length!==32){
        miiId=["10001001","10001001","11000110","01000110"][mii.general.type];//Normal, Normal, Foreign, Special
        for(var i=0;i<3;i++){
            miiId+=Math.floor(Math.random()*255).toString(2).padStart(8,"0");
        }
    }
    ...
    let systemId=mii.meta.systemId;
    if(systemId==undefined||systemId.length!==32){
        systemId="1".repeat(32);
    }
    miiBin+=systemId;
    ...
    miiBin+="000";
    if(mii.perms.mingle&&mii.general.type===3){
        mii.perms.mingle=false;
        console.error("A Special Mii cannot have Mingle on and still render on the Wii. Turned Mingle off in the output.");
    }
    miiBin+=mii.perms.mingle?"0":"1";
    miiBin+="0";
    miiBin+=mii.perms.fromCheckMiiOut?"1":"0";
    miiBin+=(+getKeyByValue(hairTable,`${mii.hair.page}${mii.hair.type}`)).toString(2).padStart(7,"0");
    miiBin+mii.hair.color.toString(2).padStart(3,"0");
    miiBin+=mii.hair.flipped?"1":"0";
    miiBin+="00000";
    miiBin+=(+getKeyByValue(eyebrowTable,`${mii.eyebrows.page}${mii.eyebrows.type}`)).toString(2).padStart(5,"0");
    miiBin+="0";
    miiBin+=mii.eyebrows.rotation.toString(2).padStart(4,"0");
    miiBin+="000000";
    miiBin+=mii.eyebrows.col.toString(2).padStart(3,"0");
    ...
    miiBin+=(+getKeyByValue(eyeTable,`${mii.eyes.page}${mii.eyes.type}`)).toString(2).padStart(6,"0");
    miiBin+="00";
    ...
    miiBin+="000";
    miiBin+=(+getKeyByValue(mouthTable,`${mii.mouth.page}${mii.mouth.type}`)).toString(2).padStart(5,"0");
    ...
    miiBin+=mii.facialHair.color.toString(2).padStart(3,"0");
    ...
    for(var i=0;i<10;i++){
        if(i<mii.creatorName.length){
            miiBin+=mii.creatorName.charCodeAt(i).toString(2).padStart(16,"0");
        }
        else{
            miiBin+="0".repeat(16);
        }
    ...
    await fs.promises.writeFile(outPath, Buffer.from(buffers));
}
async function write3DSQR(jsonIn,outPath,fflRes=_fflRes){
    if(!["3ds","wii u"].includes(jsonIn.console?.toLowerCase())){
        jsonIn=convertMii(jsonIn);
    }
    return new Promise(async (resolve) => {
        var mii=jsonIn;
        if(mii.perms.sharing&&mii.general.type===3){
            mii.perms.sharing=false;
            console.error("Cannot have Sharing enabled for Special Miis. Disabled Sharing in the output.");
        }
        var miiBin="00000011";
        miiBin+="0000000";
        ...
        for(var i=0;i<10;i++){
            if(i<mii.name.length){
                miiBin+=mii.meta.name.charCodeAt(i).toString(2).padStart(8,"0").padEnd(16,"0");
            }
            else{
                miiBin+="0".repeat(16);
            }
        ...
        miiBin+=mii.general.weight.toString(2).padStart(8,"0");
        miiBin+=mii.face.color.toString(2).padStart(3,"0");
        miiBin+=tables.faces[mii.face.type].toString(2).padStart(4,"0");
        miiBin+=mii.perms.sharing?"0":"1";
        miiBin+=mii.face.makeup.toString(2).padStart(4,"0");
        miiBin+=mii.face.feature.toString(2).padStart(4,"0");
        miiBin+=tables.hairs[mii.hair.page][mii.hair.type].toString(2).padStart(8,"0");
        miiBin+="0000";
        miiBin+=mii.hair.flipped?"1":"0";
        miiBin+=mii.hair.color.toString(2).padStart(3,"0");
        miiBin+=mii.eyes.color.toString(2).padStart(3,"0").slice(1,3);
        miiBin+=tables.eyes[mii.eyes.page][mii.eyes.type].toString(2).padStart(6,"0");
        ...
        miiBin+=tables.eyebrows[mii.eyebrows.page][mii.eyebrows.type].toString(2).padStart(5,"0");
        miiBin+="0";
        ...
        miiBin+=tables.noses[mii.nose.page][mii.nose.type].toString(2).padStart(5,"0");
        miiBin+="00";
        miiBin+=mii.nose.yPosition.toString(2).padStart(5,"0");
        miiBin+=mii.nose.size.toString(2).padStart(4,"0")[0];
        miiBin+=mii.mouth.color.toString(2).padStart(3,"0").slice(1,3);
        miiBin+=tables.mouths[mii.mouth.page][mii.mouth.type].toString(2).padStart(6,"0");
        ...
        for(var i=0;i<10;i++){
            if(i<mii.creatorName.length){
                miiBin+=mii.creatorName.charCodeAt(i).toString(2).padStart(8,"0").padEnd(16,"0");
            }
            else{
                miiBin+="0".repeat(16);
            }
        ...
        const miiBinary = Buffer.from(buffers);
        ...
        let miiPNGBuf = null;
        let renderWithStudio = fflRes===null || fflRes===undefined; 
        if(renderWithStudio){
            miiPNGBuf = await render3DSMiiWithStudio(jsonIn);
        }
        else{
            miiPNGBuf = await render3DSMii(jsonIn,fflRes);
        }
